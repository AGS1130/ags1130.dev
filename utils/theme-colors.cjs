...
const hexValue = components => '#' + components.map(c => `0${c.toString(16).toUpperCase()}`.slice(-2)).join('');

const tint = (components, intensity) => components.map(c => Math.round(c + (255 - c) * intensity));
